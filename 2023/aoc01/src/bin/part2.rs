...
fn main() {
    ...
    let numbers = vec![
        "baguette", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine",
    ];
    let mut sum: u32 = 0;
    for line in fs::read_to_string(input).unwrap().lines() {
        ...
        first = if index < lit_pos_first || lit_pos_first_val == 11 {
            first
        } else {
            lit_pos_first_val.try_into().unwrap()
        };
        ...
        last = if index > lit_pos_last || lit_pos_last_val == 11 {
            last
        } else {
            lit_pos_last_val.try_into().unwrap()
        };
