...
fn get_next_states(map: &[Vec<char>], state: &State, location: Loc) -> Vec<State> {
    ...
    loop {
        temp_state.loc_set.insert(temp_state.loc);
        // Save valid turns
        let mut temp_state_left = temp_state.clone();
        ...
        let mut temp_state_right = temp_state.clone();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        for st in next_states {
            ...
            {
                Some(x) if frontier[x].cost > st.cost => {
                    frontier.remove(x);
                    frontier.push(st);
                }
                Some(x) if frontier[x].cost == st.cost => {
                    frontier[x].loc_set.extend(st.loc_set);
                }
                Some(_) => {}
                None => {
                    match visited
                    ...
                    {
                        Some(old) => {
                            if visited[old].cost == st.cost {
                                visited[old].loc_set.extend(st.loc_set);
                            }
                        }
    ...
    for (x, line) in map.iter().enumerate() {
        for (y, ch) in line.iter().enumerate() {
            if current.loc.x == x && current.loc.y == y {
                print!("@");
            } else if current.loc_set.iter().any(|l: &Loc| x == l.x && y == l.y) {
                print!("O");
            } else {
                print!("{ch}");
            }
